  ...
  if (event->type() != QEvent::TabletLeaveProximity) {
    ...
    if (event->pointerType() == QPointingDevice::PointerType::Eraser) {
      ...
      case QInputDevice::DeviceType::Stylus:
        if (event->pointingDevice()->capabilities().testFlag(
                QPointingDevice::Capability::Rotation)) {
...
void TabletCanvas::tabletEvent(QTabletEvent *event) {
  ...
  case QEvent::TabletMove:
    ...
    if (event->pointingDevice() &&
        event->pointingDevice()->capabilities().testFlag(
            QPointingDevice::Capability::Rotation))
      updateCursor(event);
#endif
    if (m_deviceDown) {
...
void TabletCanvas::paintPixmap(QPainter &painter, QTabletEvent *event) {
  ...
  case QInputDevice::DeviceType::Airbrush: {
    ...
    update(QRect(event->position().toPoint() - QPoint(radius, radius),
                 QSize(radius * 2, radius * 2)));
  } break;
  case QInputDevice::DeviceType::Puck:
  case QInputDevice::DeviceType::Mouse: {
    const QString error(
        tr("This input device is not supported by the example."));
  ...
  case QInputDevice::DeviceType::Stylus:
    if (event->pointingDevice()->capabilities().testFlag(
      ...
      QPointF brushAdjust(
          qSin(qDegreesToRadians(-lastPoint.rotation)) * halfWidth,
          qCos(qDegreesToRadians(-lastPoint.rotation)) * halfWidth);
      poly << lastPoint.pos + brushAdjust;
      poly << lastPoint.pos - brushAdjust;
      halfWidth = m_pen.widthF();
      brushAdjust =
          QPointF(qSin(qDegreesToRadians(-event->rotation())) * halfWidth,
      ...
      update(QRect(lastPoint.pos.toPoint(), event->position().toPoint())
                 .normalized()
                 .adjusted(-maxPenRadius, -maxPenRadius, maxPenRadius,
                           maxPenRadius));
    }
    break;
  }
...
void TabletCanvas::updateBrush(const QTabletEvent *event) {
  ...
  case PressureValuator:
    ...
    break;
  case TiltValuator:
    m_pen.setWidthF(std::max(std::abs(vValue - 127), std::abs(hValue - 127)) /
                    12);
    break;
  default:
    m_pen.setWidthF(1);
